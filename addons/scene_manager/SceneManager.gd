...
func change_scene(path: PackedScene, setted_options: Dictionary = {}) -> void:
	assert(path is PackedScene, "path not packed scene")
...
func _replace_scene(path: PackedScene, options: Dictionary) -> void:
	_current_scene.queue_free()
	scene_unloaded.emit()
	_current_scene = path.instantiate()
